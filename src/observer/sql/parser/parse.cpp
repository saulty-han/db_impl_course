...
extern "C"
{
  ...
  bool check_date(int y, int m, int d)
  {
    ...
    bool m_ok = (m > 0) && (m < 13);
    bool d_ok = false;
    int ref_day[] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
    if ((y % 4 == 0 && y % 100 != 0) || (y % 400 == 0))
    {
      ref_day[1] = 29;
    }

    d_ok = (d > 0) && (d <= ref_day[m - 1]);
    if ((y >= 2038) && (m >= 2))
    {
      return 1;
    }

    return (y_ok && m_ok && d_ok);
  }
  ...
  int value_init_date(Value *value, const char *v)
  {
    ...
    if (!b)
      return -1;
    // TODO 将日期转换成整数 20020306
    int date = d + 100 * m + 10000 * y;
    // TODO 将value 的 data 属性修改为转换后的日期，强势类型转换，先分配后拷贝
