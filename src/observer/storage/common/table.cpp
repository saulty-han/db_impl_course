...
RC Table::destroy(const char *dir)
{
  ...
  // TODO 删除描述表元数据的文件，参考下面函数用法，各接口给出的还是路径文件
  std::string meta_path = table_meta_file(dir, name());
  if (unlink(meta_path.c_str()) != 0)
  {
    LOG_ERROR("Failed to drop meta file. file name=%s", meta_path.c_str());
  ...
  {
    LOG_ERROR("Failed to drop data file. file name=%s", data_path.c_str());
    return RC::GENERIC_ERROR;
  }

  // TODO 清理所有的索引相关文件数据与索引元数据，需要索引数据名
  ...
  for (int i = 0; i < index_num; i++)
  {
    ...
    {
      LOG_ERROR("Failed to drop index file. file name=%s", index_path.c_str());
      return RC::GENERIC_ERROR;
    }
  }
